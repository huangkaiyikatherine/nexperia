...
def train(train_loader, model, criterion, optimizer, epoch, timeline, es, dataset, mod=None, crop='center'):
    ...
    if dataset=='nexperia':
        ...
        for i, ((input, target, index, image_id), idx, sample_idx) in enumerate(train_loader):
            ...
            target[torch.logical_and(sample_idx==1, target==9)] = 3
            target[torch.logical_and(sample_idx==1, target==8)] = 9
...
def validate(val_loader, model, epoch, timeline, dataset, state=None, criterion=None, last=False, crop='center'):
    ...
    for i, ((input, target, index, image_id), idx, sample_idx) in enumerate(val_loader):
        input = input.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        sample_idx = sample_idx.cuda(non_blocking=True)
