...
parser = argparse.ArgumentParser(description='Huber Lasso training')
# network
...
parser.add_argument('--lambd', default=0.5, type=float,
                    help='huber lasso regularization weight')
# misc
...
parser.add_argument('--huber-lasso', default=True, type=bool,
                    help='whether to add huber-lasso regularization')
...
def main():
    ...
    distance_from_corrupted_labels = np.zeros((len(targets),args.epochs))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        distance = train(train_loader, model, criterion, optimizer, epoch, args.lambd, len(targets), args.huber_lasso)
        distance_from_corrupted_labels[:,epoch]=distance
        print("*" * 40)
    ...
    df = pd.DataFrame(distance_from_corrupted_labels)
    filename = '{}_{}_{}_{}_r{}_lambda{}_{}_huber_lasso_delta.csv'.format(
        args.dataset, args.arch, args.loss, args.noise_type, args.noise_rate, args.lambd, args.lr_schedule)
    np.savetxt(os.path.join('/home/kaiyihuang/giants_shoulder/self-adaptive-training-master/logs',filename),
               df, delimiter=",")
...
def train(train_loader, model, criterion, optimizer, epoch, lambd, data_size, reg=True):
    """
        Run one train epoch
    """
    ...
    for i, (input, target, index) in enumerate(train_loader):
        ...
        if reg:
            ...
            delta = 1 - torch.gather(prob,1,target.reshape(-1,1))
            distance[index]=delta.cpu().detach().numpy().reshape(-1,)
        else:
            delta = 0
        loss = criterion(output, target) + lambd * torch.norm(delta, p=1)
