...
parser.add_argument('--data-root-2', help='The directory of data',
                    default='~/datasets/CIFAR10', type=str)
parser.add_argument('--data-root-3', help='The directory of data',
                    default='~/datasets/CIFAR10', type=str)
...
parser.add_argument('--train-set', help='subsets that used to training',
                    default='train', type=str)
parser.add_argument('--val-set', type=str, default='noisy_val',
                    help='subsets that used to validation')
parser.add_argument('--test-set', type=str, default='noisy_test',
                    help='subsets that used to test')
...
def main():
    ...
    # prepare dataset
    train_loader, val_loaders, test_loader, num_classes, image_datasets = get_loader(args)
    ...
    criterion = get_loss(args, num_classes=num_classes, datasets=image_datasets)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, num_classes, train_timeline, args.sat_es, args.dataset, args.mod, args.crop)
        print("*" * 40)
        ...
        print('val:')
        val_auc = validate(
            val_loaders, model, epoch, num_classes, val_timeline, args.dataset, state='val', criterion=criterion, crop=args.crop)
        print("*" * 40)

        print('test:')
        test_auc = validate(
            test_loader, model, epoch, num_classes, test_timeline, args.dataset, state='test', criterion=criterion, crop=args.crop)
        print("*" * 40)
    ...
    validate(test_loader, model, epoch, num_classes, test_timeline, args.dataset, last=True, crop=args.crop)
    print("*" * 40)
    ...
    validate(test_loader, model, checkpoint['epoch'], num_classes, test_timeline, args.dataset, last=True, crop=args.crop)
    print("*" * 40)
...
def train(train_loader, model, criterion, optimizer, epoch, num_classes, timeline, es, dataset, mod=None, crop='center'):
    """
        Run one train epoch
    """
    ...
    for i, ((input, target, sub_index, image_id), index, dataset_idx) in enumerate(train_loader):
        ...
        dataset_idx = dataset_idx.cuda(non_blocking=True)

        target[torch.logical_and(dataset_idx==1, target==9)] = 3
        target[torch.logical_and(dataset_idx==1, target==8)] = 9
        target[torch.logical_and(dataset_idx==2, target==7)] = 10
        target[torch.logical_and(dataset_idx==2, target==6)] = 7
        target[torch.logical_and(dataset_idx==2, target==5)] = 6
        target[torch.logical_and(dataset_idx==2, target==4)] = 5
        target[torch.logical_and(dataset_idx==2, target==8)] = 4
    ...
    acc_class = torch.zeros(1, num_classes)
    loss_class = torch.zeros(1, num_classes)
    acc_bi_class = torch.zeros(1, num_classes)
    loss_bi_class = torch.zeros(1, num_classes)
    me_class = torch.zeros(1,num_classes)
    me_bi_class = torch.zeros(1,num_classes)
    ...
    len_train = criterion.len_train
    len_val = criterion.len_val
    len_test = criterion.len_test

    for i in range(num_classes):
        if torch.sum(criterion.image_class_index[:len_train]==i).item()!=0:
            acc_class[0, i] = torch.sum(torch.argmax(outputs[:len_train][
                criterion.image_class_index[:len_train]==i], dim=1)==i).item() / np.count_nonzero(
                criterion.image_class_index[:len_train].cpu()==i)
            loss_class[0, i] = -torch.mean(torch.log(outputs[:len_train][
                criterion.image_class_index[:len_train]==i,i])).item()
            outputs_class = outputs[:len_train][criterion.image_class_index[:len_train]==i]
            me_class[0, i] = torch.mean(outputs_class[:,i] - torch.max(outputs_class[:,np.arange(num_classes)!=i], dim=1)[0]).item()
            me_bi_class[0, i] = torch.mean((outputs_class[:,4] * 2 - 1) * ((i==4) - 0.5)) * 2.
            if i!=4:
                acc_bi_class[0, i] = torch.sum(outputs[:len_train][
                    criterion.image_class_index[:len_train]==i,4]<=0.5).item() / np.count_nonzero(
                    criterion.image_class_index[:len_train].cpu()==i)
                loss_bi_class[0, i] = -torch.mean(torch.log(1-outputs[:len_train][
                    criterion.image_class_index[:len_train]==i,4])).item()
            else:
                acc_bi_class[0, i] = torch.sum(outputs[:len_train][
                    criterion.image_class_index[:len_train]==i,4]>0.5).item() / np.count_nonzero(
                    criterion.image_class_index[:len_train].cpu()==i)
                loss_bi_class[0, i] = -torch.mean(torch.log(outputs[:len_train][
                criterion.image_class_index[:len_train]==i,4])).item()
...
def validate(val_loader, model, epoch, num_classes, timeline, dataset, state=None, criterion=None, last=False, crop='center'):
    """
    Run evaluation
    """
    batch_time = AverageMeter()
    ...
    for i, ((input, target, sub_index, image_id), index, dataset_idx) in enumerate(val_loader):
        input = input.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        dataset_idx = dataset_idx.cuda(non_blocking=True)
    ...
    if state is not None:
        ...
        for i in range(10):
            if torch.sum(criterion.image_class_index[start:end]==i).item()!=0:
                acc_class[0, i] = torch.sum(torch.argmax(outputs[start:end][
                    criterion.image_class_index[start:end]==i,:], dim=1)==i).item() / np.count_nonzero(
                    criterion.image_class_index[start:end].cpu()==i)
                loss_class[0, i] = -torch.mean(torch.log(outputs[start:end][
                    criterion.image_class_index[start:end]==i,i])).item()
                outputs_class = outputs[start:end][criterion.image_class_index[start:end]==i]
                me_class[0, i] = torch.mean(outputs_class[:,i] - torch.max(outputs_class[:,np.arange(num_classes)!=i], dim=1)[0]).item()
                me_bi_class[0, i] = torch.mean((outputs_class[:,4] * 2 - 1) * ((i==4) - 0.5)) * 2.
                if i!=4:
                    acc_bi_class[0, i] = torch.sum(outputs[start:end][
                        criterion.image_class_index[start:end]==i,4]<=0.5).item() / np.count_nonzero(
                        criterion.image_class_index[start:end].cpu()==i)
                    loss_bi_class[0, i] = -torch.mean(torch.log(1-outputs[start:end][
                    criterion.image_class_index[start:end]==i,4])).item()
                else:
                    acc_bi_class[0, i] = torch.sum(outputs[start:end][
                        criterion.image_class_index[start:end]==i,4]>0.5).item() / np.count_nonzero(
                        criterion.image_class_index[start:end].cpu()==i)
